    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-3xl font-black text-slate-900 tracking-tight italic uppercase leading-none">Matriz de Asignación</h2>
          </div>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-13">Vincule identidades corporativas con niveles de privilegio</p>
        </div>
        <Button
          onClick={() => setModalUserRol({ isOpen: true, userRol: undefined })}
          className="bg-emerald-950 hover:bg-emerald-900 text-white font-black px-8 py-7 rounded-2xl shadow-xl shadow-emerald-100 transition-all hover:scale-105 active:scale-95 text-[11px] uppercase tracking-widest gap-3"
        >
      ...
      <div className="relative group">
        ...
        <Input
          ...
          className="pl-14 py-8 bg-white border-2 border-slate-100 rounded-[2.2rem] text-sm font-bold focus-visible:ring-emerald-600/5 focus-visible:border-emerald-600 transition-all shadow-sm placeholder:text-slate-300 placeholder:uppercase placeholder:tracking-widest placeholder:text-[9px]"
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
        />
        <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
           <div className="h-10 w-px bg-slate-100 mx-2" />
           <Badge variant="outline" className="h-10 px-6 border-slate-100 text-slate-400 font-black text-[10px] uppercase tracking-widest bg-slate-50/50 rounded-2xl hidden md:flex items-center">
      ...
      <div className="grid grid-cols-1 gap-6">
        {filteredUserRoles.length === 0 ? (
          <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-[4rem] p-24 text-center animate-in zoom-in duration-500">
            ...
            <h3 className="text-2xl font-black text-slate-900 uppercase italic mb-3 tracking-tight">Repositorio Libre de Asignaciones</h3>
                ...
                <div className="flex items-center gap-8 flex-1 min-w-0 relative z-10">
                  ...
                  <div className="space-y-4 flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-4">
                       <h3 className="font-black text-slate-900 uppercase tracking-tighter text-2xl group-hover:text-emerald-900 transition-colors italic leading-none truncate">
                       ...
                       <div className="flex items-center gap-2 bg-emerald-50 px-4 py-2 rounded-xl group-hover:bg-emerald-600 group-hover:text-white transition-all duration-500">
                         ...
                         <span className="text-[10px] font-black uppercase tracking-[0.2em]">{userRol.rol_nombre}</span>
                       </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-y-3 gap-x-8">
                      ...
                      <div className="flex items-center gap-3 text-slate-400 group-hover:text-slate-600 transition-colors">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest">UID-X: {userRol.user_id}</span>
                      </div>
                      <div className="flex items-center gap-3 text-slate-400 group-hover:text-slate-600 transition-colors">
                        <div className="h-7 w-7 rounded-lg bg-slate-50 flex items-center justify-center">
                          <Database className="w-3.5 h-3.5 text-slate-300 group-hover:text-emerald-400" />
                        </div>
                        <span className="text-[10px] font-black uppercase tracking-widest text-nowrap">REF: #{userRol.id}</span>
                      </div>
                    </div>
                  </div>
                </div>
